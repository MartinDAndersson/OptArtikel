...
"""
    choose_best_mode(V, state, time, current_mode, cost, payoff, dt, J)

Choose the optimal mode to switch to given the current state and continuation values.

V now returns ĝ_j (continuation value per target mode j). The decision is:
    argmax_j [ f_j·dt - c_{current_mode, j} + ĝ_j ]

Parameters:
- V: Continuation value function ĝ_j = V(state, time, j)
- state: Current state vector
- time: Current time
- current_mode: Current operating mode
- cost: Switching cost function cost(state, time)[from_mode, to_mode]
- payoff: Payoff function payoff(state, time) returning vector of payoffs per mode
- dt: Time step
...
"""
function choose_best_mode(V, state, time, current_mode, cost, payoff, dt, J)
    f = payoff(state, time*dt) .* dt
    values = Vector{Float64}(undef, J)
    for j in 1:J
        values[j] = f[j] + V(state, time, j)[1] - cost(state, time*dt)[current_mode, j]
    end
...
function plot_value_prediction_error(Vs::Vector, sample_paths::Array{Float64,3}, 
    ...
    for (idx, V) in enumerate(Vs)
        predicted_values, strategy = compute_strategy_for_value(
            V, sample_paths, times, cost, payoff, dt, initial_mode, J)
...
function compute_strategy_for_value(V, sample_paths, times, cost, payoff, dt, initial_mode, J)
    ...
    Threads.@threads for path in 1:num_paths
        ...
        for t in 1:num_times
            state = sample_paths[:, t, path]
            if t < num_times
                # Reconstruct V̂_i = max_j [f_j·dt - c_{i,j} + ĝ_j]
                f = payoff(state, times[t]*dt) .* dt
                c = cost(state, times[t]*dt)
                g = [V(state, times[t], j)[1] for j in 1:J]
                predicted_values[t, path] = maximum(f .+ g .- c[current_mode, :])
                best_mode = choose_best_mode(V, state, times[t], current_mode, cost, payoff, dt, J)
